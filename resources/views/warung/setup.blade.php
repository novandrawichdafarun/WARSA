            ...
            <form method="POST" action="{{ route('warung.setup.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="alamat" rows="2" placeholder="Jl. Contoh No. 1, Surabaya"
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent resize-none">{{ old('alamat') }}</textarea>
                </div>
                ...
                <button type="submit"
                    class="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded-lg transition-colors shadow-sm">
                    Buat Warung & Mulai →
                </button>
            </form>
        </div>
